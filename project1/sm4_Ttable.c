...
// 使用T-table优化的T函数
static uint32_t T_opt(uint32_t x) {
    return T_table[(x >> 24) & 0xFF][0] ^
           T_table[(x >> 16) & 0xFF][1] ^
           T_table[(x >> 8) & 0xFF][2] ^
           T_table[x & 0xFF][3];
}
...
// 密钥扩展函数
static void sm4_key_expansion_opt(const uint8_t *key, uint32_t rk[32]) {
    uint32_t MK[4];
    uint32_t K[36];
    int i;
    ...
    // 生成轮密钥
    for (i = 0; i < 32; i++) {
        K[i+4] = K[i] ^ T_opt(K[i+1] ^ K[i+2] ^ K[i+3] ^ CK[i]);
        rk[i] = K[i+4];
    }
}
...
void sm4_encrypt_opt(const uint8_t *key, const uint8_t *input, uint8_t *output) {
    ...
    // 初始化T-table（实际应用中应只初始化一次）
    init_T_table();
    
    // 密钥扩展
    sm4_key_expansion_opt(key, rk);
